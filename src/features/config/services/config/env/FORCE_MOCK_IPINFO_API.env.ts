...
export class FORCE_MOCK_IPINFO_API implements ConfigEnvironmentEntry {
    addEnvironment(configBuilder: ConfigBuilder): ConfigBuilder {
        const config: ConfigEnvConfig = {
            description: "Determines whether the IPINFO API endpoint should be forcefully mocked.",
            secret: false,
            type: TypeStrings.BOOLEAN
        };

        return configBuilder
            .addEnvKey(Configs.FORCE_MOCK_IPINFO_API, config);
    }
}
