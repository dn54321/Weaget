...
export class FORCE_MOCK_OPENWEATHER_API implements ConfigEnvironmentEntry {
    addEnvironment(configBuilder: ConfigBuilder): ConfigBuilder {
        const config: ConfigEnvConfig = {
            description: "Determines whether the Openweather API endpoint should be forcefully mocked.",
            secret: false,
            type: TypeStrings.BOOLEAN
        };

        return configBuilder
            .addEnvKey(Configs.FORCE_MOCK_OPENWEATHER_API, config);
    }
}
