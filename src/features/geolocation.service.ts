...
import { GeonamesNearbyLocation } from "@src/apis/geonames/nearby-location/nearby-location.types";
import GoogleLocationSuggestion from "@src/apis/google/location-autocomplete/location-auto-complete.model";
...
export async function getNearbyLocationDetails(
    ...
    const response = await fetch(geonamesLocationLookupUrl, {
        next: {
            revalidate: LOCATION_LOOKUP_CACHE_SECONDS,
            tags: ["geonames::nearby-search"]
        }
    });
