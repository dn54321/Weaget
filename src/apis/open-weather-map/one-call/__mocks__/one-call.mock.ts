...
export function createOpenWeatherHourlyMockData(override?: object) {
    return {
        clouds: faker.number.int({ max: 100, min: 0 }),
        dew_point: faker.number.float({ fractionDigits: 2, max: 331, min: 186 }),
        dt: faker.number.int(),
...
export function createOpenWeatherMinutelyMockData(override?: object) {
    return {
        dt: faker.number.int(),
...
export function createOpenWeatherOneCallMockData() {
    ...
    return {
        ...
        daily: faker.helpers.multiple((_, idx) => createOpenWeatherDailyMockData({ dt: currentDate + idx * 86400 }), { count: 8 }),
        hourly: faker.helpers.multiple((_, idx) => createOpenWeatherHourlyMockData({ dt: currentDate + idx * 3600 }), { count: 48 }),
        lat: faker.location.longitude({ precision: 4 }),
        lon: faker.location.latitude({ precision: 4 }),
        minutely: faker.helpers.multiple((_, idx) => createOpenWeatherMinutelyMockData({ dt: currentDate + idx * 60 }), { count: 60 }),
        timezone: timezone,
