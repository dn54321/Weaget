...
export class ApinPollutionGetMockController {
    createApicnPollutionAttributionMockData() {
        return {
    ...
    createApicnPollutionResultMockData() {
        const randDate = DateTime.fromJSDate(faker.date.anytime());
        return {
            aqi: faker.number.int({ max: 200, min: 0 }),
            attributions: faker.helpers.multiple(this.createApicnPollutionAttributionMockData, { count: { max: 3, min: 1 } }),
            city: {
                geo: [
            ...
            forecast: {
                daily: {
                    o3: faker.helpers.multiple(this.createApicnPollutionRollingAverage, { count: 7 }),
                    pm10: faker.helpers.multiple(this.createApicnPollutionRollingAverage, { count: 7 }),
                    pm25: faker.helpers.multiple(this.createApicnPollutionRollingAverage, { count: 7 })
                }
            },
            iaqi: {
                co: {
    ...
    createApicnPollutionRollingAverage() {
    ...
    async  generateMock() {
        return {
            data: this.createApicnPollutionResultMockData(),
            status: "ok"
        };
    }
}
