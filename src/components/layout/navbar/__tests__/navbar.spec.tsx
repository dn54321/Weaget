...
import { useMobileScreen } from "@src/utils/resize-window";
...
describe("Component: navbar", () => {
    ...
    it("should have access to the search bar in mobile view.", async () => {
        const user = userEvent.setup();
        fireEvent(window, new Event("resize"));
        const { getByPlaceholderText, getByRole } = withRender(<Navbar />);
    ...
    it("should be able to close the search bar in mobile view.", async () => {
        const user = userEvent.setup();
        useMobileScreen();
        const { getByLabelText, getByRole } = withRender(<Navbar />);
